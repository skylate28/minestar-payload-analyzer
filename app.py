...
# --- JUDUL APLIKASI ---
st.title("🚛 Minestar Payload Anomaly Detector")
st.markdown(f"""
...
if uploaded_file is not None:
    try:
        ...
        if not set(required_columns).issubset(df.columns):
            ...
            # TABEL DATA
            st.subheader("📋 Daftar Unit Bermasalah")
            ...
            if not df_anomalies.empty:
                ...
                st.download_button(
                    label="📥 Download Data Anomali (CSV)",
                    data=csv,
                ...
                st.success("✅ Tidak ada anomali ditemukan! Semua unit beroperasi sesuai standar.")

    except Exception as e:
        st.error(f"Terjadi kesalahan saat memproses file: {e}")
else:
    # Tampilan Awal jika belum upload
    st.info("👈 Silahkan upload file CSV Minestar di panel sebelah kiri untuk memulai analisa.")
    st.markdown("---")
